...
local_model_path = os.environ['FAST'] + "/rpisano1/models/qwen_7b_instruct"
...
Step 2: ...
...
Put the choosen option within \\boxed{{}}."""
			messages = [{'role': 'user', 'content': prompt}]
			input_list = [apply_chat_template(tokenizer, messages)]
			N_input_list = input_list * N
			prompt_token_ids.extend(N_input_list)
		...
		for i, problem in enumerate(problems):
			...
			for gen in generations_i:
				...
				if num_matches_steps > 0 and answer is not None and not gen.outputs[0].text in generated_critiques_i:
					...
					continue
					# print(f"Skipping generation for problem {problem} due to format issues.")
					# print("\n\n#########################################################################################################\n\n")
	...
	def process_dataset(dataset, ds_name, n=8):
		...
		alphabet_map = {chr(i + 65): i for i in range(26)}
		...
		for batch_idx in range(processed_batches, total_batches):
			...
			for i in range(len(question)):
				problems.append(f"{question[i]}. Choose one of the following options: {options[i]}")
			answers = batch['answer']
			solutions = []
			for i in range(len(answers)):
				solution_position = alphabet_map[answers[i]]
				if solution_position >= len(options[i]):
					raise ValueError(f"Solution position {solution_position} out of range for options {options[i]}, answer {answers[i]}, question {question[i]}")
				solutions.append(options[i][solution_position])
	...
	ds_local = load_dataset("json", data_files={
		"test": datasets_dir + "MMLU-Pro.json.json",
	})

	process_dataset(ds_local['test'], "MMLU-Pro.json", n=8)
